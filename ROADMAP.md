...
1.5 ⬜ **Economy core (decided 2026-09-25, plan `Docs/Plans/04-economy-core.md`)** — before the meta tree, because upgrades are priced against income. Steps: measure the real economy with headless simulations; change match payout to match-power; build the gatherer haul loop + gather spots (**built 2026-09-26**); build raider walk/swing targeting; re-measure and set costs from "purchases per round" targets. The Mine/Refinery/metal-tier chain (design doc §11.3) is *vision* that belongs with item 2, not part of this.
...
| B8 | Resource costs / income numbers feel off | **Diagnosed 2026-09-25:** matching pays 1 item per tile (3× the reference game) and gatherers collect in place forever (~30 items per battle vs ~5–6 with a haul loop). Fix = economy core (build order 1.5). **Step 1 measured 2026-09-25** (see wiki → Economy → Income): skilled matching ≈ 32 items/round vs random ≈ 5; 1 Farmer ≈ 29 items/battle; Mine 1.2 iron/s, builds in 2s, doesn't persist; raiders often overwhelm 2 Knights. | 🟡 in progress |
...
| 2026-09-25 | **Match payout = match power (3→1, 4→2, 5→3 items per group); dust stays 1 per destroyed tile** | Matches the reference game the developer replayed; small starting numbers make each extra tile matter more. Prices are set independently of income (from "purchases per round" targets). **Built 2026-09-25** (`MatchingSystem._compute_group_rewards`, tests in `MatchingRulesTest`). |
| 2026-09-26 | **Gatherer haul loop BUILT (plan 04 step 3)** | `GatherSpots` (4 spots/tile, dead holders count as free), `GatherBrain` state machine, `GatherComponent.collect/drop_off`, `UnitDef.gather_time` (3.0, was `gather_interval`) + `carry_capacity` (1). Killed while carrying loses the load. New `GatherLoopTest`; all 12 suites green. Re-measured: 1 Farmer ~6.3 items/battle (was 29.3), 4 Farmers ~23.4; with raiders 3.8 / 20.3. Prices not yet re-set (step 5). |
